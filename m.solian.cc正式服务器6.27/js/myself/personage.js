...
$(function(){
	...
	$.ajax({
        ...
        success: function(data){
            ...
            if(personage.image=="" || personage.image==null){
            	$(".d_inpImg").attr("src","../../img/icon/d_head.png");
            }else{
...
//			});
            ...
            if(localStorage.getItem("upImg")!=null){
            	...
            	var url=d_http+"index.php/Home/Article/upload_myimage";
            	 _ajax.upLoadImg(localStorage.getItem("upImg"),uid,url,function(data){
            	 	
            	 	if(data.code==200){
            	 		$(".d_inpImg").attr("src",data.data)
            	 		localStorage.removeItem("upImg");
            	 		$(".d_popupbox").html("头像修改成功")
						setTimeout(show,500)
            ...
            $(".d_inpImg").on("click",function(){
            	location.href="../../src/login/upLoad.html"
            })
            //点击昵称
	...
	$(".d_line9").on("click",function(){
		...
		$(".clickon3").on("click",function(){
			if($(".d_windowbg3 input").val()<140 || $(".d_windowbg3 input").val()>210){
	...
	$(".d_save").on("click",function(){
			$.ajax({
				...
				success:function(data){
					if($(".d_text9").html=="cm" || $(".d_text9").html==""){
						$(".d_popupbox").html("请填写你的身高")
						setTimeout(show,500)
						setTimeout(hide,3000)
					}else if($(".d_text10").html=="kg" || $(".d_text10").html==""){
						$(".d_popupbox").html("请填写你的体重")
						setTimeout(show,500)
						setTimeout(hide,3000)
