		...
		success:function(data){
			if(data.code==200){
				...
				console.log(obj)
					...
					$(".d_datatitle span").text(obj.object);
				...
				if(d_type==3){
							...
							$(".d_metodata").on("click",function(){//点击我要和Ta约会
								$.ajax({
									...
									success:function(data){
										if(data.code==200){
											$(".d_tanbox").hide();//弹框消失
											$(".d_sureone").show();//被邀请人员具体信息布局
											$(".d_beizhu").show();
											var successdata=data.data;
											$(".d_dataxiny span").text(successdata.credit);//约会信用值
											$(".d_headtop img").attr("src",successdata.image);//头像
											$(".d_nickname").text(successdata.nickname);//昵称
											$(".d_age").text(successdata.age);//年龄
											$(".d_address").text(successdata.city);//地址
											if(successdata.marry==1){
												$(".d_marry").text("单身");//单身
											}else if(successdata.marry==1){
												$(".d_marry").text("已婚");//已婚
											}else if(successdata.marry==1){
												$(".d_marry").text("情侣");//情侣
											}
											if(successdata.sex==1){//1男2女
												$(".d_seximg").attr("src","../../img/icon/t_boy.png");
											}else{
												$(".d_seximg").attr("src","../../img/icon/t_girl.png");
											}
											if(successdata.old_uid==0){//身份是否验证
												$(".d_shenfen").attr("src","../../img/icon/x_approve_icon.png");
											}else{
												if(successdata.sex==1){
													$(".d_shenfen").attr("src","../../img/icon/x_boyapprove_icon.png");
												}else{
													$(".d_shenfen").attr("src","../../img/icon/x_girlapprove_icon.png");
												}
											...
											$(".d_datain p").text("Ta的粉丝");//粉丝
											$(".d_datain span").text(successdata.fans);//粉丝
											var successlist=successdata.state;
											$(".d_datejindumore").empty();
											for(var i in successlist){
												$(".d_datejindumore").append("<li><p class='d_jinduDian'></p><span>"+successlist[i].state_list+"</span><p class='d_jindutime'>"+format(successlist[i].state_time).substring(0,16)+"</p></li>")
											}
										}
									}
								});
					...
					$(".d_moreheadimg").on("click",function(){
						location.href="../../src/myself/morepeople.html?id="+id+"&otheruid="+obj.uid
					})
				}
			}
			/* 约会进度 */
