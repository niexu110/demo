...
$(function() {
	...
	$(".more").on("click", function() {  $(".cover").show();  })
		//关闭弹框
	$(".close").on("click", function() {   $(".cover").hide();  });
	...
	_ajax.getAjax(url, datas, "POST", function(data) {
		if(data.code == 200) {
			...
			list.type == 1 ? (list.typeText = "我请客", list.typeImg = "../../img/n_md.png") :
				(list.type==2?(list.typeText = "AA制", list.typeImg = "../../img/n_AA.png"):(list.typeText = "请我吧", list.typeImg = "../../img/n_md.png"));
			...
			$("#scene").click(function() {
				location.href = "businessInfo_join.html?id=" + list.goods_id;
			})
			$("#joinBtn").click(function() {
				if(userObj == null) {
					localStorage.setItem("url", window.location.href)
