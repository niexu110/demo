 ...
 */
$(function(){
    var slider=$(".slider"),iSlider=$(".iSlider"),
     opacity=$(".pso_img"),
    ...
    var index=0,index2=0,index1=0;
    opacity.eq(index1).css({opacity:"1"});
   setInterval(function(){
        if(index1>opacity.length){
            index1=0;
            opacity.eq(index1).css({opacity:"1",transition:"all 0s linear"}).siblings().css({opacity:"0"});
        }else{
            index1++;
            opacity.eq(index1).css({opacity:"1",transition:"all 1s linear"}).siblings().css({opacity:"0"});
        }
    },3000);
    function fn(){
